...
public class CommandHandler
{
    ...
    {
        var comms = typeof(CommandHandler).Assembly.GetTypes().Where(type =>
            type is { IsClass: true, IsAbstract: false } && type.GetCustomAttribute(typeof(CommandAttribute)) != null);
...
public class GenericHelpCommand() : ConsoleCommand("help", allowOnTitle: true)
{
    ...
    public override void Handle(string[] args)
    {
        ...
        {
            text =
                $"The '{CommandHandler.RootCommand}' command is the command prefix for {CommandHandler.ModName} commands. To use a {CommandHandler.ModName} command, type '{CommandHandler.RootCommand}' followed by the command name and any applicable arguments.\r\n\r\n" +
                $"Available commands:";
        ...
        {
            text =
                $"The '{CommandHandler.RootCommand} {args[0]}' command is not a valid {CommandHandler.ModName} command.";
        }
